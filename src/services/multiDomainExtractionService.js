...
function extractPayrollSections(sheetName, rows) {
    ...
    return [makeSection({
        ...
        questions: security ? [
            question('postingTarget', 'How should this security sheet be posted?', ['outsourced_staff_attendance', 'vendor_bill', 'both_attendance_and_vendor_bill', 'reference_only'], 'reference_only'),
            question('vendorMatch', 'Which security-service vendor should be matched?', ['ask_user_to_select_existing_vendor', 'create_new_vendor', 'do_not_create_vendor'], 'ask_user_to_select_existing_vendor')
        ] : [
            question('postingTarget', 'How should this payroll sheet be posted?', ['draft_payroll_run', 'employee_salary_history', 'attendance_only', 'reference_only'], 'reference_only'),
            question('employeeMatching', 'How should employee names be matched?', ['business_id_then_name', 'name_and_designation', 'manual_review_each_unmatched'], 'manual_review_each_unmatched')
...
function extractAccountSections(sheetName, rows) {
    ...
    for (const region of accountHeaderRegions(rows)) {
        ...
        for (let r = region.row + 1; r < rows.length; r++) {
            ...
            records.push({ sourceRow: r + 1, date, party, ledgerHead, purpose: text(valueAt(rows, r, map.purpose)), particular, voucherNumber: text(valueAt(rows, r, map.voucher)), debit: rounded(debit), credit: rounded(credit), balance: rounded(amount(valueAt(rows, r, map.balance))) });
        }
        if (records.length < 2) continue;
        sections.push(makeSection({ type: 'account_transactions', title: `${sheetName} account transactions`, sheetName, range: `row ${region.row + 1}, columns ${region.start + 1}-${region.end + 1}`, confidence: 0.97, records, columns: Object.keys(records[0]),
            summary: { transactions: records.length, debit: rounded(records.reduce((s, r) => s + r.debit, 0)), credit: rounded(records.reduce((s, r) => s + r.credit, 0)), closingBalance: records.at(-1)?.balance || 0 },
            questions: [question('accountMatch', 'Which ERP cash/bank account does this ledger represent?', ['ask_user_to_select_account', 'create_draft_account', 'reference_only'], 'reference_only'), question('postingTarget', 'How should transactions be handled?', ['draft_for_accounts_review', 'reference_only', 'skip_section'], 'draft_for_accounts_review')], postingMode: 'department_draft' }));
    }
    return sections;
}
...
function entityCandidates(sections) {
    const map = new Map();
    const add = (name, source, suggestedRole, confidence) => { const clean = text(name); if (!clean || clean.length < 3) return; const key = norm(clean); const current = map.get(key) || { id:key, name:clean, sources:[], suggestedRole, confidence, selected:true, role:'', matchBusinessId:'' }; if (!current.sources.includes(source)) current.sources.push(source); if (confidence > current.confidence) { current.suggestedRole=suggestedRole; current.confidence=confidence; } map.set(key,current); };
    for (const section of sections) {
        ...
        if (section.type === 'account_transactions') for (const row of section.records) if (row.party && !/^(cash|bank|atm|opening|office expenses?)$/i.test(row.party)) add(row.party, section.sheetName, 'unresolved_party', 0.55);
    }
...
function buildMultiResult(originalName,sourceHash,sheets,sections) {
    ...
    for(const section of sections){if(fingerprints.has(section.fingerprint)){section.selected=false;section.duplicateOf=fingerprints.get(section.fingerprint);duplicates.push({sectionId:section.id,duplicateOf:section.duplicateOf});}else fingerprints.set(section.fingerprint,section.id);}
