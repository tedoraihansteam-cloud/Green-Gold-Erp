...
async function referenceRegister(req,res){const values=[req.user.company_id],where=[`r.company_id=$1`];if(req.query.q){values.push(`%${String(req.query.q).trim()}%`);where.push(`(r.record_data::text ILIKE $${values.length} OR r.section_title ILIKE $${values.length} OR j.original_name ILIKE $${values.length})`);}if(req.query.disposition){values.push(req.query.disposition);where.push(`r.disposition=$${values.length}`);}const page=Math.max(1,Number(req.query.page)||1),limit=Math.min(500,Math.max(25,Number(req.query.limit)||100)),base=`FROM bulk_import_reference_rows r JOIN bulk_import_jobs j ON j.id=r.job_id WHERE ${where.join(' AND ')}`,total=Number((await query(`SELECT count(*) count ${base}`,values)).rows[0].count);values.push(limit,(page-1)*limit);const {rows}=await query(`SELECT r.*,j.business_id import_business_id,j.original_name ${base} ORDER BY r.effective_date DESC NULLS LAST,r.created_at DESC LIMIT $${values.length-1} OFFSET $${values.length}`,values);res.json({rows,total,page,limit,pages:Math.ceil(total/limit)});}
