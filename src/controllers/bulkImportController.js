...
async function canonicalReferenceEntity(client,{companyId,businessId,type,name,path={}}){
  let entity=(await client.query(`SELECT id FROM bulk_import_reference_entities WHERE company_id=$1 AND (business_id=$2 OR linked_business_id=$2) ORDER BY linked_business_id IS NOT NULL DESC LIMIT 1`,[companyId,businessId])).rows[0];
  if(entity){await client.query(`UPDATE bulk_import_reference_entities SET linked_business_id=COALESCE(linked_business_id,$2),organization_path=organization_path||$3::jsonb WHERE id=$1`,[entity.id,businessId,JSON.stringify(path)]);return entity;}
  try{return (await client.query(`INSERT INTO bulk_import_reference_entities(company_id,business_id,entity_type,display_name,linked_business_id,organization_path) VALUES($1,$2,$3,$4,$2,$5::jsonb) ON CONFLICT(company_id,entity_type,display_name) DO UPDATE SET linked_business_id=COALESCE(bulk_import_reference_entities.linked_business_id,EXCLUDED.linked_business_id),organization_path=bulk_import_reference_entities.organization_path||EXCLUDED.organization_path RETURNING id`,[companyId,businessId,type,name||businessId,JSON.stringify(path)])).rows[0];}
  catch(error){if(error.code!=='23505')throw error;entity=(await client.query(`SELECT id FROM bulk_import_reference_entities WHERE company_id=$1 AND (business_id=$2 OR linked_business_id=$2) LIMIT 1`,[companyId,businessId])).rows[0];if(entity)return entity;throw error;}
}
async function linkReferenceEntities(client,job){for(const item of (job.extraction_result?.entityCandidates||[]).filter(x=>x.selected!==false)){const type=item.matchEntityType||({customer:'CUSTOMER',vendor:'VENDOR',staff:'EMPLOYEE',account:'ACCOUNT',external_person:'EXTERNAL_PERSON',both:'CUSTOMER_VENDOR'}[item.role])||'OTHER',businessId=item.matchBusinessId||`REF-${type}-${String(item.id||require('crypto').randomUUID()).replace(/[^a-z0-9]/gi,'').slice(-12).toUpperCase()}`,path={branchBusinessId:item.branchBusinessId||null,departmentBusinessId:item.departmentBusinessId||null,siteId:item.siteId||null,locationBusinessId:item.locationBusinessId||null};const entity=await canonicalReferenceEntity(client,{companyId:job.company_id,businessId,type,name:item.name,path});await client.query(`INSERT INTO bulk_import_reference_links(reference_row_id,reference_entity_id) SELECT r.id,$1 FROM bulk_import_reference_rows r WHERE r.job_id=$2 AND r.record_data::text ILIKE $3 ON CONFLICT DO NOTHING`,[entity.id,job.id,`%${String(item.name||'').replaceAll('%','')}%`]);}}
async function linkExplicitDestinations(client,job){const typeMap={accountBusinessId:'ACCOUNT',customerBusinessId:'CUSTOMER',vendorBusinessId:'VENDOR',employeeBusinessId:'EMPLOYEE',warehouseBusinessId:'WAREHOUSE',locationBusinessId:'LOCATION',departmentBusinessId:'DEPARTMENT'};for(const section of job.extraction_result?.sections||[]){for(const [key,type] of Object.entries(typeMap)){const businessId=section.postingOptions?.[key];if(!businessId)continue;const name=(await client.query(`SELECT COALESCE((SELECT name FROM accounts WHERE company_id=$1 AND business_id=$2),(SELECT name FROM master_customers WHERE company_id=$1 AND business_id=$2),(SELECT name FROM master_vendors WHERE company_id=$1 AND business_id=$2),(SELECT full_name FROM master_employees WHERE company_id=$1 AND business_id=$2),(SELECT name FROM warehouses WHERE company_id=$1 AND business_id=$2),(SELECT name FROM storage_locations WHERE company_id=$1 AND business_id=$2),(SELECT d.name FROM departments d JOIN branches b ON b.id=d.branch_id WHERE b.company_id=$1 AND d.business_id=$2),$2) display_name`,[job.company_id,businessId])).rows[0].display_name;const entity=await canonicalReferenceEntity(client,{companyId:job.company_id,businessId,type,name});await client.query(`INSERT INTO bulk_import_reference_links(reference_row_id,reference_entity_id,relationship) SELECT id,$1,'selected_destination' FROM bulk_import_reference_rows WHERE job_id=$2 AND section_id=$3 ON CONFLICT DO NOTHING`,[entity.id,job.id,section.id]);}}}
...
async function upload(req, res) {
    ...
    try {
        ...
        if(analysis.sourceSummary?.sourceHash){try{await query(`DELETE FROM bulk_import_jobs j WHERE j.company_id=$1 AND j.source_summary->>'sourceHash'=$2 AND j.status IN('review','rejected') AND NOT EXISTS(SELECT 1 FROM bulk_import_reference_rows r WHERE r.job_id=j.id)`,[req.user.company_id,analysis.sourceSummary.sourceHash]);}catch(cleanupError){console.warn('Skipped non-critical duplicate import cleanup:',cleanupError.message);}}
...
async function referenceRegister(req,res){const values=[req.user.company_id],where=[`r.company_id=$1`];if(req.query.q){values.push(`%${String(req.query.q).trim()}%`);where.push(`(r.record_data::text ILIKE $${values.length} OR r.section_title ILIKE $${values.length} OR j.original_name ILIKE $${values.length} OR EXISTS(SELECT 1 FROM bulk_import_reference_links ql JOIN bulk_import_reference_entities qe ON qe.id=ql.reference_entity_id WHERE ql.reference_row_id=r.id AND (qe.display_name ILIKE $${values.length} OR qe.business_id ILIKE $${values.length} OR qe.linked_business_id ILIKE $${values.length})))`);}if(req.query.disposition){values.push(req.query.disposition);where.push(`r.disposition=$${values.length}`);}const page=Math.max(1,Number(req.query.page)||1),limit=Math.min(500,Math.max(25,Number(req.query.limit)||100)),base=`FROM bulk_import_reference_rows r JOIN bulk_import_jobs j ON j.id=r.job_id WHERE ${where.join(' AND ')}`,total=Number((await query(`SELECT count(*) count ${base}`,values)).rows[0].count);values.push(limit,(page-1)*limit);const {rows}=await query(`SELECT r.*,j.business_id import_business_id,j.original_name,COALESCE((SELECT jsonb_agg(DISTINCT jsonb_build_object('businessId',COALESCE(e.linked_business_id,e.business_id),'name',e.display_name,'type',e.entity_type,'relationship',l.relationship)) FROM bulk_import_reference_links l JOIN bulk_import_reference_entities e ON e.id=l.reference_entity_id WHERE l.reference_row_id=r.id),'[]'::jsonb) linked_profiles ${base} ORDER BY r.effective_date DESC NULLS LAST,r.created_at DESC LIMIT $${values.length-1} OFFSET $${values.length}`,values);res.json({rows,total,page,limit,pages:Math.ceil(total/limit)});}
async function hydrateReferenceJob(client,job){const {rows}=await client.query(`SELECT section_id,record_data FROM bulk_import_reference_rows WHERE job_id=$1 AND disposition IN('current_operational','scheduled_future') ORDER BY source_row`,[job.id]);const grouped=rows.reduce((map,row)=>{(map[row.section_id]||=[]).push(row.record_data);return map;},{});return {...job,extraction_result:{...(job.extraction_result||{}),sections:(job.extraction_result?.sections||[]).map(section=>({...section,records:grouped[section.id]||[]}))}};}
function compactApprovedExtraction(extraction){return {...extraction,entityCandidates:[],sections:(extraction?.sections||[]).map(({records,sourceSnapshot,manualReview,...section})=>({...section,recordCount:records?.length||0}))};}
async function entityReferences(req,res){const {rows}=await query(`SELECT r.*,j.business_id import_business_id,j.original_name,e.business_id reference_business_id,e.entity_type,e.display_name,e.organization_path entity_path FROM bulk_import_reference_entities e JOIN bulk_import_reference_links l ON l.reference_entity_id=e.id JOIN bulk_import_reference_rows r ON r.id=l.reference_row_id JOIN bulk_import_jobs j ON j.id=r.job_id WHERE e.company_id=$1 AND (e.linked_business_id=$2 OR e.business_id=$2) ORDER BY r.effective_date DESC NULLS LAST,r.created_at DESC LIMIT 500`,[req.user.company_id,req.params.businessId]);res.json({references:rows});}
...
async function decideApproval(req, res) {
    ...
    const result = await withTransaction(async (client) => {
        ...
        await preserveReferenceRows(client,job);await linkReferenceEntities(client,job);await linkExplicitDestinations(client,job);
        ...
        const updated = (await client.query(`UPDATE bulk_import_jobs SET status='submitted',final_approved_by=$1,final_approved_at=now(),approval_notes=$2,submission_result=$3::jsonb,submitted_by=$1,submitted_at=now(),preview_rows='[]'::jsonb,detected_columns='[]'::jsonb,extraction_result=$4::jsonb WHERE id=$5 RETURNING *`, [req.user.id, notes, JSON.stringify(summary),JSON.stringify(compactApprovedExtraction(job.extraction_result)), job.id])).rows[0];
...
async function postApprovedResults(req, res) {
    const result = await withTransaction(async (client) => {
        ...
        const hydrated=await hydrateReferenceJob(client,job);const routed = await routeApprovedMultiDomain(client, hydrated, req.user);
...
module.exports = { upload, list, get, referenceRegister, entityReferences, updateMapping: updateReview, submitForApproval, decideApproval, postApprovedResults, submit, template, remove };
