...
async function upload(req, res) {
    ...
    try {
        ...
        if(analysis.sourceSummary?.sourceHash)await query(`DELETE FROM bulk_import_jobs j WHERE j.company_id=$1 AND j.source_summary->>'sourceHash'=$2 AND j.status IN('review','rejected') AND NOT EXISTS(SELECT 1 FROM bulk_import_reference_rows r WHERE r.job_id=j.id)`,[req.user.company_id,analysis.sourceSummary.sourceHash]);
