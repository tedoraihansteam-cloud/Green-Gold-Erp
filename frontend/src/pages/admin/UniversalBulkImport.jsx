...
export default function UniversalBulkImport() {
    ...
    return <section className="card">
        ...
        <p className="hint">Historical reference keeps extracted facts independently and never changes or validates live balances. Operational posting is separate and enforces account, stock, and transaction continuity.</p>
        <details className="card" open style={{padding:14,marginBottom:14}}><summary><strong>Historical reference register</strong> · searchable rows linked to ERP profiles</summary><div className="field" style={{marginTop:12}}><label>Search reference data or linked business ID</label><input value={referenceQuery} onChange={event=>setReferenceQuery(event.target.value)} placeholder="Account, customer, product, voucher, workbook, business ID or amount"/></div><div className="hint" style={{marginBottom:10}}>Evidence only: these rows do not add money, deduct stock, create dues, or require opening and closing balances to reconcile.</div><div style={{overflow:'auto',maxHeight:480}}><table className="data"><thead><tr><th>Import</th><th>Workbook / sheet</th><th>Linked profiles</th><th>Source</th><th>Classification</th><th>Date</th><th>Extracted data</th></tr></thead><tbody>{(referenceData?.rows||[]).map(row=><tr key={row.id}><td>{row.import_business_id}</td><td>{row.original_name}<div className="hint">{row.sheet_name}</div></td><td>{(row.linked_profiles||[]).length?(row.linked_profiles||[]).map(profile=><div key={`${profile.type}-${profile.businessId}`}><strong>{profile.businessId}</strong><div className="hint">{profile.type} · {profile.name}</div></div>):<span className="hint">Not linked</span>}</td><td>{row.source_row}</td><td>{String(row.disposition).replaceAll('_',' ')}</td><td>{row.effective_date||'—'}</td><td><pre style={{whiteSpace:'pre-wrap',maxWidth:500}}>{JSON.stringify(row.record_data,null,2)}</pre></td></tr>)}</tbody></table></div></details>
