...
export default function AccountsPage() {
    ...
    return (
        <div>
                ...
                {loading ? <p style={{ color: 'var(--ink-600)' }}>Loading…</p> : (
                    <DataTable
                        columns={[
                            ...
                            {key:'actions',label:'',render:r=>can('ACCOUNTS_CREATE')?<button className="btn btn-secondary btn-sm" onClick={()=>setEditingAccount({businessId:r.business_id,name:r.name,bankName:r.bank_name||'',bankAccountNumber:r.bank_account_number||'',openingBalance:r.opening_balance??0})}>Edit setup</button>:null}
                        ]}
                        rows={accounts}
            ...
            {editingAccount&&<Modal title="Edit account setup" onClose={()=>setEditingAccount(null)}><form onSubmit={async e=>{e.preventDefault();try{await api.put(`/accounts/${editingAccount.businessId}`,editingAccount);setEditingAccount(null);reload();}catch(x){alert(x.message)}}}><div className="field"><label>Account name *</label><input required value={editingAccount.name} onChange={e=>setEditingAccount({...editingAccount,name:e.target.value})}/></div><div className="form-grid"><div className="field"><label>Bank name</label><input value={editingAccount.bankName} onChange={e=>setEditingAccount({...editingAccount,bankName:e.target.value})}/></div><div className="field"><label>Account number</label><input value={editingAccount.bankAccountNumber} onChange={e=>setEditingAccount({...editingAccount,bankAccountNumber:e.target.value})}/></div><div className="field"><label>Opening balance</label><input type="number" min="0" step="0.01" value={editingAccount.openingBalance} onChange={e=>setEditingAccount({...editingAccount,openingBalance:Number(e.target.value)})}/><div className="hint">Editable only during setup. It locks after any operational transaction or finance go-live.</div></div></div><div className="form-actions"><button className="btn btn-primary">Save account setup</button></div></form></Modal>}
